...
@section('content')
    ...
    <div class="pub-head">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
            <a href="https://www.youtube.com/@arvitaaguskurniasari434" target="_blank" rel="noopener noreferrer" class="pub-btn"
                style="background:#18382c;">
                YouTube Channel
            </a>
            <form method="POST" action="{{ route('admin.videos.sync-youtube') }}" class="inline">
                @csrf
                <button type="submit"
                    class="inline-flex items-center rounded-2xl bg-emerald-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-emerald-950 dark:bg-amber-200 dark:text-stone-950 dark:hover:bg-amber-100">
                    Sync YouTube
                </button>
            </form>
    ...
    <form method="GET" action="{{ route('admin.videos.index') }}" class="pub-panel pub-filter">
        <input type="search" name="q" value="{{ request('q') }}" class="pub-input"
